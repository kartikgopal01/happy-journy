...
export default function AdminHotelsPage() {
  ...
  async function downloadTemplate() {
    try {
      const response = await fetch("/api/admin/hotels/download-template");
      if (response.ok) {
        const blob = await response.blob();
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = "hotels-template.xlsx";
  ...
  async function handleBulkImport(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/admin/hotels/bulk-import", {
        method: "POST",
        body: formData,
      });
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1">
              <label htmlFor="excel-upload-hotels" className="cursor-pointer">
                <input
                  type="file"
                  accept=".xlsx,.xls"
